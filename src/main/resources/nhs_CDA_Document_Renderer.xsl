...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:n1="urn:hl7-org:v3" xmlns:npfitlc="NPFIT:HL7:Localisation" xmlns:n2="urn:hl7-org:v3/meta/voc" xmlns:voc="urn:hl7-org:v3/voc" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" exclude-result-prefixes="n1 n2 voc npfitlc xsi">
	...
	<xsl:template match="n1:ClinicalDocument">
		<html>
			<head>
				<xsl:comment>$transformVersion=2.1 $updateDate=22/02/2013 Interoperability Team</xsl:comment>	
          ...
          Document. </xsl:comment>
			...
			<body>
				<div style="color:red; font-weight:bold;">***This HTML is created using a transform which is provided on an as is basis, and the content of this HTML is not clinically validated. The transform used to create this HTML can be modified as per local needs.***</div>
					...
					<tr>
						...
						<td>
							...
							<!-- Unverified NHS Number -->
								...
								<!--PRTR1 Logic to get patient id status (only for unverified NHS number)-->
								<xsl:if test="/n1:ClinicalDocument/n1:recordTarget/n1:patientRole/n1:id[@root='2.16.840.1.113883.2.1.3.2.4.18.23']/@patientIdStatus">
									<xsl:text> [</xsl:text> 
										<xsl:call-template name="translateStatusCode">
											<xsl:with-param name="statusCode" select="/n1:ClinicalDocument/n1:recordTarget/n1:patientRole/n1:id[@root='2.16.840.1.113883.2.1.3.2.4.18.23']/@patientIdStatus"/>
										</xsl:call-template>
									<xsl:text>] </xsl:text>								
								</xsl:if>									
								<br/>
...
<!--PRTR1 Participants ends here-->
  ...
  -->
   ...
   <xsl:template name="translateLanguageCode">
	...
	<!--PRTR1 Support for translation of NHS Status code-->
   <xsl:template name="translateStatusCode">
      <xsl:param name="statusCode"/>
      <xsl:choose>
		<!--PRTR1 Added Status Code Mapping-->
		<xsl:when test="$statusCode='02'"><xsl:text>Number present but not traced</xsl:text></xsl:when>		
		<xsl:when test="$statusCode='03'"><xsl:text>Trace required</xsl:text></xsl:when>		
		<xsl:when test="$statusCode='04'"><xsl:text>Trace attempted</xsl:text></xsl:when>		
		<xsl:when test="$statusCode='05'"><xsl:text>Trace needs to be resolved</xsl:text></xsl:when>		
		<xsl:when test="$statusCode='06'"><xsl:text>Trace in progress</xsl:text></xsl:when>		
		<xsl:when test="$statusCode='08'"><xsl:text>Trace postponed (baby &lt; 6 weeks old)</xsl:text></xsl:when>		
         <xsl:otherwise>
